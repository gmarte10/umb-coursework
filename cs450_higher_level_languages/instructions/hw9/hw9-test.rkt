...
(define test-lookup
  (test-case "Exercise 1. lookup"
    (check-equal? (s:eval 'x.y (hash 'x (box (hash "y" 10)))) 10)))

(define test-update
  (test-case "Exercise 1. update"
    ;; Update
    (let [(x (box (hash "y" 10)))]
...
(define test-full
  (test-case "Exercise 1. full system"
