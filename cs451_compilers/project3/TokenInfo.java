...
enum TokenKind {
    ...
    INTERFACE("interface"), LONG("long"), SWITCH("switch"), THROW("throw"),
    THROWS("throws"), TRY("try"),

    // Operators.
    ASSIGN("="), DEC("--"), EQUAL("=="), GT(">"), INC("++"), LAND("&&"),
    LE("<="), LNOT("!"), MINUS("-"), PLUS("+"), PLUS_ASSIGN("+="), STAR("*"),
    DIV("/"), REM("%"), ALSHIFT("<<"), ARSHIFT(">>"), LRSHIFT(">>>"), NOT("~"),
    AND("&"), XOR("^"), OR("|"), QUESTION("?"), NOT_EQUAL("!="), DIV_ASSIGN("/="),
    MINUS_ASSIGN("-="), STAR_ASSIGN("*="), REM_ASSIGN("%="),
    ARIGHTSHIFT_ASSIGN(">>="), LRSHIFT_ASSIGN(">>>="), GE(">="), ALSHIFT_ASSIGN("<<="), LT("<"),
    XOR_ASSIGN("^="), OR_ASSIGN("|="), LOR("||"), AND_ASSIGN("&="),
