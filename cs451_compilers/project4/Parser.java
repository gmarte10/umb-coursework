...
public class Parser {
    ...
    /**
     * Parses forUpdate and returns a list of JStatements.
     *
     * <pre>
     *   forUpdate ::= statementExpression {COMMA statementExpression}
     * </pre>
     *
     * @return a list of JStatements.
     */
    ...
    /**
     * Parses forInit and returns a list of JStatements.
     *
     * <pre>
    ...
    private ArrayList<JStatement> forInit() {
        ...
        if (!seeLocalVariableDeclaration()) {
            ...
            Type type = type();
            JVariableDeclaration jVariableDeclaration= new JVariableDeclaration(line, variableDeclarators(type));
            init.add(jVariableDeclaration);
        }
        return init;
    }

    /**
     * Parses conditionalOrExpression and returns a JExpression.
     *
     * <pre>
    ...
    private JExpression conditionalOrExpression2() {
        int line = scanner.token().line();
        JExpression condAndExpress = conditionalAndExpression();
        while(have(LOR)) {
            condAndExpress = new JLogicalOrOp(line, condAndExpress, conditionalAndExpression());
        }
        return condAndExpress;
    }
     ...
     *               | Do statementExpression WHILE parExpression SEMI
     *               | FOR LPAREN [forInit] SEMI [expresison] SEMI [forUpdate] RPAREN statement
     *               | BREAK SEMI
     *               | CONTINUE SEMI
...
//        else if (see(QUESTION)) {
//            JExpression condOrExpr = conditionalOrExpression();
//            if (have(QUESTION)) {
//                JExpression expression = expression();
//                mustBe(COLON);
//                JExpression condExpr = expression();
//                return new JConditionalExpression(line, condOrExpr, expression, condExpr);
//            }
//            return new JConditionalExpression(line, condOrExpr, null, null);
//        }
        // Do statement
        else if (have(DO)) {
            JStatement statement = statement();
            mustBe(WHILE);
            JExpression pe = parExpression();
            mustBe(SEMI);
            return new JDoStatement(line, statement, pe);
        }
        // For statement
        else if (have(FOR)) {
            mustBe(LPAREN);
            if (see(SEMI)) {
                if (see(SEMI)) {
                    if (see(RPAREN)) {
                        ...
                        return new JForStatement(line, null, null, null, body);
                    } else {
                        mustBe(RPAREN);
                        JStatement body = statement();
                        return new JForStatement(line, null, null, null, body);
                    ...
                    mustBe(RPAREN);
                    JStatement body = statement();
                    return new JForStatement(line, null, condition, update, body);
                }
            } else {
        ...
        // Exception Handlers
            ...
            JExpression jExpression = expression();
            mustBe(SEMI);
            return new JThrowStatement(line, jExpression);
        } else if (have(TRY)) {
     ...
     *                                [ ( ASSIGN | PLUS_ASSIGN | MINUS_ASSIGN | STAR_ASSIGN | DIV_ASSIGN | REM_ASSIGN
     *                                | LRSHIFT_ASSIGN | ALSHIFT_ASSIGN | ARSHIFT_ASSIGN | AND_ASSIGN | XOR_ASSIGN
     *                                | OR_ASSIGN) assignmentExpression ]
     * </pre>
     *
    ...
    private JExpression conditionalExpression() {
        int line = scanner.token().line();
        JExpression jorexp = conditionalOrExpression();
        if (have(QUESTION)) {
            JExpression expression = expression();
            mustBe(COLON);
            JExpression condExp = conditionalOrExpression();
            return new JConditionalExpression(line, jorexp, expression, condExp);
        }
        return new JConditionalExpression(line, jorexp, null, null);
    }
    ...
    private JExpression assignmentExpression() {
        ...
        } else if (have(ARIGHTSHIFT_ASSIGN)) {
     ...
     *
     * <pre>
     *   multiplicativeExpression ::= unaryExpression { (STAR | DIV | REM
     *   | ALSHIFT | ARSHIFT | LRSHIFT | AND | XOR | OR) unaryExpression }
     * </pre>
     *
     ...
     *
     * <pre>
     *   literal ::= CHAR_LITERAL | FALSE | INT_LITERAL | DOUBLE_LITERAL | LONG_LITERAL | NULL | STRING_LITERAL | TRUE
     * </pre>
     *
