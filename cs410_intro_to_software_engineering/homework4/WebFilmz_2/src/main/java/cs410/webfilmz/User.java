...
public class User implements ILikeFilm {
    ...
    // Remove watched films from year, most liked and most watched recommendation sets.
    public Map<String, Set<Film>> getAllRecommendations(Catalog catalog, int initialGenericRecsCount) {
        ...
        // Remove films that are not within the acceptable rating limit
        recommendationsByYear.removeIf(film -> !film.isAppropriateFor(limitRating));
        recommendationsByMostLiked.removeIf(film -> !film.isAppropriateFor(limitRating));
        recommendationsByMostWatched.removeIf(film -> !film.isAppropriateFor(limitRating));
        recommendationsByGenre.removeIf(film -> !film.isAppropriateFor(limitRating));
        recommendationsByDirector.removeIf(film -> !film.isAppropriateFor(limitRating));
